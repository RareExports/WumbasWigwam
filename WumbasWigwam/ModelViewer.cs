...
namespace WumbasWigwam
{
  public class ModelViewer : Form
  {
    ...
    private byte[] vertexColorData = new byte[1];
    private float[] vertexTextureCoordData = new float[1];
    private List<TextureData> TextureDataList = new List<TextureData>();
    private List<Texture> TextureSettings = new List<Texture>();
    private List<int> TextureSettingsBuffer = new List<int>();
    private int newx;
    private int newy;
    private int oldx;
    private int oldy;
    ...
    private string replacementAnimation = "";
    public List<int> updatedModels = new List<int>();
    public List<int> updatedAnimations = new List<int>();
    ...
    private ToolStripMenuItem loadAnimationFileToolStripMenuItem;
    private ToolStripMenuItem saveAnimationToRomToolStripMenuItem;
      ...
      ref List<SetupFile> levels_)
    {
      ...
      this.tmpDir = tmpDir_;
      this.outDir = outDir_;
      this.exportToGeObjToolStripMenuItem.Enabled = false;
    }
    ...
    private void readObjectsXML()
    {
      ...
      this.models.Add(new ObjectData("Banjo-Kazooie", (short) 0, (short) 0, 1578, 0, "", "", 0, 0));
    }
    ...
    private void camera()
    {
      ...
      {
        Texture textureSetting = this.TextureSettings[this.TextureSettingsBuffer[index]];
        if (textureSetting.glname == -1)
        {
        ...
        else
        {
          ...
          GL.BindTexture(TextureTarget.Texture2D, textureSetting.glname);
        }
        GL.Disable(EnableCap.CullFace);
        if (textureSetting.cullMode == CullMode.CULLFRONT)
          GL.CullFace(CullFaceMode.Front);
        else if (textureSetting.cullMode == CullMode.CULLBACK)
          GL.CullFace(CullFaceMode.Back);
        else if (textureSetting.cullMode == CullMode.CULLBOTH)
          GL.CullFace(CullFaceMode.FrontAndBack);
        if (textureSetting.cullMode != CullMode.CULLNONE)
      ...
      GL.PopMatrix();
      Core.SetView(this.BKOpenGLC.Height, this.BKOpenGLC.Width);
      this.BKOpenGLC.SwapBuffers();
    }
    ...
    private void PrepareModel(int pointer)
    {
      this.bytesInFile = File.ReadAllBytes(this.tmpDir + pointer.ToString("x"));
      this.PrepareModel();
    }
    ...
    private void ripVertBoneAssignments(ref byte[] modelfile)
    {
      ...
      if (num3 != 0)
      {
        ...
        while (index1 < num2)
        {
          int index2 = ((int) modelfile[index1] << 8) + (int) modelfile[index1 + 1];
          int index3 = index1 + 2;
          int num4 = ((int) modelfile[index3] << 8) + (int) modelfile[index3 + 1];
          int index4 = index3 + 2;
          int num5 = num4 * 6;
          int num6 = ((int) modelfile[index4] << 8) + (int) modelfile[index4 + 1];
          int num7 = index4 + 2;
          List<int> intList = new List<int>();
          int num8 = num5 + (num6 << 2);
          for (index1 = num7 + num8; index1 < modelfile.Length; index1 += 2)
          {
            short num9 = (short) (((int) modelfile[index1] << 8) + (int) modelfile[index1 + 1]);
            if (num9 < (short) -1)
            {
              index1 += 2;
              break;
            ...
            if (num9 != (short) -1)
              intList.Add((int) num9);
          }
          this.skeleton[index2].verts.AddRange((IEnumerable<int>) intList);
        }
      }
    ...
    private void loadReplacementModelToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      if (this.openFileDialog1.ShowDialog() == DialogResult.OK)
      {
        ...
        Core.DeleteBuffer(this.vboVertexHandle);
        Core.DeleteBuffer(this.vboColorHandle);
        Core.DeleteBuffer(this.vboTexCoordHandle);
        Core.DeleteBuffers(this.iboHandles);
        foreach (Texture textureSetting in this.TextureSettings)
        {
          if (textureSetting.glname != -1)
          {
            uint glname = (uint) textureSetting.glname;
            GL.DeleteTextures(1, ref glname);
          }
        }
        this.iboData.Clear();
        ...
        this.bytesInFile = File.ReadAllBytes(this.replacementModel);
        if (this.objects_dgv.SelectedRows.Count > 0)
        {
          ...
          this.PrepareModel();
          Core.GetBuffersFromBKModelFile(ref this.bytesInFile, ref this.vboVertexHandle, ref this.vertexData, ref this.vboColorHandle, ref this.vertexColorData, ref this.vboTexCoordHandle, ref this.vertexTextureCoordData, ref this.iboHandles, ref this.iboData, ref this.TextureDataList, ref this.TextureSettings, ref this.TextureSettingsBuffer);
          this.DisplayAnimationFiles(this.o.file);
          this.forceRedraw = true;
          this.BBCamera.Reset();
        }
      }
    ...
    private void objects_dgv_SelectionChanged(object sender, EventArgs e)
    {
      ...
      try
      {
        RomHandler.DecompressFileToHDD(this.o.Pointer);
      }
      catch (Exception ex)
      {
      }
      ...
      this.exportToGeObjToolStripMenuItem.Enabled = true;
    }
    ...
    private void levels_dgv_SelectionChanged(object sender, EventArgs e)
    {
      ...
      try
      {
        RomHandler.DecompressFileToHDD(this.levelPointer);
      }
      catch (Exception ex)
      {
      }
    ...
    private void RenderFrame(int frameNumber, bool customAnimation = false)
    {
      if (!(this.animation_dgv.SelectedRows.Count > 0 | customAnimation))
        return;
      foreach (Bone bone in this.skeleton)
    ...
    private void animation_dgv_CellClick(object sender, DataGridViewCellEventArgs e)
    {
      ...
      try
      {
        RomHandler.DecompressFileToHDD(animation.pointer);
      }
      catch (Exception ex)
      {
      }
      ...
      this.lbl_animation.Text = animation.id.ToString("x");
          ...
          for (int startFrame = (int) this.animation.startFrame; startFrame <= (int) this.animation.endFrame; ++startFrame)
          {
            ...
            if (this.o != null)
            {
              if (this.isDLAnimation)
                this.vertexFrames.Add(Core.DrawDLAnimationFrameVBO(ref this.bytesInFile, this.skeleton, this.translationFactor, this.vertexData));
              else
                this.vertexFrames.Add(Core.DrawVertAnimationFrameVBO(ref this.bytesInFile, this.skeleton, this.translationFactor, (float[]) this.vertexData.Clone()));
            }
            stopwatch.Stop();
    ...
    private void exportToGeObjToolStripMenuItem_Click(object sender, EventArgs e)
    {
      if (this.folderBrowserDialog.ShowDialog() != DialogResult.OK)
        return;
      Core.TranslateToGEOBJ(this.folderBrowserDialog.SelectedPath + "\\", (this.levelPointer == 0 ? this.o.Pointer : this.levelPointer).ToString("x"), this.vertexData, this.vertexColorData, this.vertexTextureCoordData, this.TextureDataList, this.TextureSettings, this.TextureSettingsBuffer, this.iboData);
    ...
    private void loadAnimationFileToolStripMenuItem_Click(object sender, EventArgs e)
    {
      this.vertexFrames.Clear();
      if (this.openFileDialog1.ShowDialog() != DialogResult.OK)
        return;
      this.replacementAnimation = this.openFileDialog1.FileName;
      this.animation = new BKAnimation(File.ReadAllBytes(this.replacementAnimation));
      if ((int) this.animation.endFrame - (int) this.animation.startFrame <= 2000 && (int) this.animation.endFrame - (int) this.animation.startFrame > 0)
      {
        this.frame_spin.Maximum = (Decimal) this.animation.endFrame;
        this.frame_spin.Minimum = (Decimal) this.animation.startFrame;
        ...
        try
        {
          ...
          for (int startFrame = (int) this.animation.startFrame; startFrame <= (int) this.animation.endFrame; ++startFrame)
          {
            this.RenderFrame(startFrame, true);
            Stopwatch.StartNew();
            if (this.o != null)
            {
              if (this.isDLAnimation)
    ...
    private void animation_dgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
    {
    }
    ...
    private void saveToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      try
      {
        ...
        if (File.Exists(this.replacementModel))
        {
          ...
          File.Delete(outDir + str);
        }
        string replacementModel = this.replacementModel;
        string outDir1 = this.outDir;
        pointer = model.Pointer;
        string str1 = pointer.ToString("x");
        string destFileName = outDir1 + str1;
        File.Copy(replacementModel, destFileName);
      }
      catch
      {
      }
    ...
    private void saveAnimationToRomToolStripMenuItem_Click(object sender, EventArgs e)
    {
      if (this.animation_dgv.SelectedRows.Count <= 0)
        return;
      if (!(this.replacementAnimation != ""))
        return;
      try
      {
        AnimationFile animation = this.animations[(int) this.animation_dgv.SelectedRows[0].Cells[0].Value];
        if (!this.updatedAnimations.Contains(animation.pointer))
          this.updatedAnimations.Add(animation.pointer);
        if (File.Exists(this.replacementAnimation))
          File.Delete(this.outDir + animation.pointer.ToString("x"));
        File.Copy(this.replacementAnimation, this.outDir + animation.pointer.ToString("x"));
      }
      catch
      {
      }
    ...
    private void InitializeComponent()
    {
      ...
      this.loadAnimationFileToolStripMenuItem = new ToolStripMenuItem();
      this.saveAnimationToRomToolStripMenuItem = new ToolStripMenuItem();
      ...
      this.fileToolStripMenuItem.DropDownItems.AddRange(new ToolStripItem[7]
      {
        ...
        (ToolStripItem) this.loadAnimationFileToolStripMenuItem,
        (ToolStripItem) this.saveAnimationToRomToolStripMenuItem
      });
      ...
      this.loadReplacementModelToolStripMenuItem.Visible = false;
      ...
      this.loadAnimationFileToolStripMenuItem.Size = new Size(209, 22);
      this.loadAnimationFileToolStripMenuItem.Text = "Load Animation File";
      this.loadAnimationFileToolStripMenuItem.Visible = false;
      this.loadAnimationFileToolStripMenuItem.Click += new EventHandler(this.loadAnimationFileToolStripMenuItem_Click);
      this.saveAnimationToRomToolStripMenuItem.Name = "saveAnimationToRomToolStripMenuItem";
      this.saveAnimationToRomToolStripMenuItem.Size = new Size(209, 22);
      this.saveAnimationToRomToolStripMenuItem.Text = "Save Animation to Rom";
      this.saveAnimationToRomToolStripMenuItem.Visible = false;
      this.saveAnimationToRomToolStripMenuItem.Click += new EventHandler(this.saveAnimationToRomToolStripMenuItem_Click);
      ...
      this.animation_dgv.CellContentClick += new DataGridViewCellEventHandler(this.animation_dgv_CellContentClick);
      ...
      this.panel2.Controls.Add((Control) this.lbl_animation);
      ...
      this.lbl_animation.Location = new Point(565, 6);
      this.lbl_animation.Name = "lbl_animation";
      this.lbl_animation.Size = new Size(52, 13);
      this.lbl_animation.TabIndex = 82;
